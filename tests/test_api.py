...
class APITestCase(unittest.TestCase):
    ...
    def test_posts(self):
        r = Role.query.filter_by(name=u'사용자').first()
        self.assertIsNotNone(r)
        u = User(email='john@example.com', password='12345', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()

        response = self.client.post(url_for('api.new_post'), \
                    headers=self.get_api_headers('john@example.com', '12345'), data=json.dumps(\
        ...
        response = self.client.get(url, \
                    headers=self.get_api_headers('john@example.com', '12345'))
        self.assertTrue(response.status_code == 200)
