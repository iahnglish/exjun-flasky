...
def index():
	...
	current_time=datetime.utcnow(), form=form, posts=posts, show_followed=show_followed, \
...
def edit_profile_admin(id):
	...
	if form.validate_on_submit():
		...
		user.confirmed = form.username.confirmed
		user.role = Role.query.get(form.role.data)
		user.name = form.name.data
		user.location = form.location.data
		user.about_me = form.about_me.data
		db.session.add(user)
		flash(u'프로파일이 수정되었습니다.')
...
def unfollow(username):
	...
	if not current_user.is_following(user):
		flash(u'현재 팔로윙하고있지 않습니다.')
		return redirect(url_for('.user', username=username))
	current_user.unfollow(user)
	flash(u'%s를 언팔로우되었습니다.' % username)
	return redirect(url_for('.user', username=username))
...
def followers(username):
	...
	return render_template('followers.html', user=user, title=u'님의 팔로워들', endpoint='.followers', pagination=pagination,\
	 follows=follows)
...
def followed_by(username):
	...
	return render_template('followers.html', user=user, title=u'님이 팔로윙', endpoint='.followed_by', pagination=pagination,\
	follows=follows)
...
def post(id):
	...
	paginate(page, per_page=current_app.\
	config['FLASKY_COMMENTS_PER_PAGE'], error_out=False)
...
def moderate():
	...
	paginaiton=pagination, page=page)
