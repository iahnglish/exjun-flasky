...
class Role(db.Model):
	...
	def insert_roles():
		roles = { 'User' : (Permission.FOLLOW | Permission.COMMENT | Permission.WRITE_ARTICLES, True),
					...
					u'사용자' : (Permission.FOLLOW | Permission.COMMENT | Permission.WRITE_ARTICLES, True),
					u'중간관리자' : (Permission.FOLLOW | Permission.COMMENT | Permission.WRITE_ARTICLES | Permission.MODERATE_COMMENTS, False),
					u'관리자' : (0xff, False)
				}
		for r in roles:
...
class User(UserMixin, db.Model):
	...
	@property
	def password(self):
		raise AttributeError(u'비밀번호를 읽을 수 없습니다.')
